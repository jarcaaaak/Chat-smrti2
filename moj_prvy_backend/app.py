...
# Inicializuj databázu pri štarte
init_db()
...
def chat(id):
    ...
    # Vymaž históriu tejto osoby pri každom otvorení chatu
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM conversations WHERE person_id = %s", (id,))
    conn.commit()
    cur.close()
    conn.close()
...
Odpovedaj výhradne v slovenčine. Buď priateľský, prirodzený a historicky presný."""
        ...
        # Načítaj históriu z databázy
        conn = get_conn()
        cur = conn.cursor()

        cur.execute(
            "SELECT role, content FROM conversations WHERE person_id = %s ORDER BY created_at ASC",
            (person_id,)
        )
        history = [{"role": row[0], "content": row[1]} for row in cur.fetchall()]

        # Ulož správu užívateľa do DB
        cur.execute(
            "INSERT INTO conversations (person_id, role, content) VALUES (%s, %s, %s)",
            (person_id, "user", user_message)
        )
        conn.commit()

        history.append({"role": "user", "content": user_message})

        # Orezanie histórie na posledných 16 správ
        if len(history) > 16:
            history = history[-16:]
        ...
        # Ulož odpoveď AI do DB
        cur.execute(
            "INSERT INTO conversations (person_id, role, content) VALUES (%s, %s, %s)",
            (person_id, "assistant", reply)
        )
        conn.commit()
        cur.close()
        conn.close()
